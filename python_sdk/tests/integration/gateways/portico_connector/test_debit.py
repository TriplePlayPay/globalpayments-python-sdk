...
from python_sdk.globalpayments.api.entities.encryption_data import EncryptionData
from python_sdk.globalpayments.api.entities.enums import PaymentMethodType
from python_sdk.globalpayments.api.entities.exceptions import (
    UnsupportedTransactionException,
)
from python_sdk.globalpayments.api.payment_methods.debit import DebitTrackData
...
class IntegrationGatewaysPorticoConnectorDebitTests(unittest.TestCase):
    ...
    def test_debit_cannot_refund_from_transaction_id_only(self):
        with self.assertRaises(UnsupportedTransactionException):
            Transaction.from_id(
                "1234567890", PaymentMethodType.Debit
            ).refund().with_currency("USD").execute("debit")

    def test_debit_cannot_reverse_from_transaction_id_only(self):
        with self.assertRaises(UnsupportedTransactionException):
            Transaction.from_id(
                "1234567890", PaymentMethodType.Debit
            ).reverse().with_currency("USD").execute("debit")
