...
from python_sdk.globalpayments.api.entities.exceptions import BuilderException
...
class AuthorizationBuilderValidationTests(unittest.TestCase):
    ...
    def test_credit_auth_no_payment_method(self):
        with self.assertRaises(BuilderException):
            self.card.authorize(14).with_currency("USD").with_payment_method(
                None
            ).execute()

    def test_credit_auth_no_validation_error(self):
        try:
            self.card.authorize(14).with_currency("USD").with_allow_duplicates(
                True
            ).execute()
        except BuilderException:
            self.fail("Properly configured auth builder should not raise exception")
    ...
    def test_credit_sale_no_payment_method(self):
        with self.assertRaises(BuilderException):
            self.card.charge(14).with_currency("USD").with_payment_method(
                None
            ).execute()

    def test_credit_offline_no_amount(self):
        with self.assertRaises(BuilderException):
            self.card.authorize().with_offline_auth_code("12345").execute()

    def test_credit_offline_no_currency(self):
        with self.assertRaises(BuilderException):
            self.card.authorize(14).with_offline_auth_code("12345").execute()

    def test_credit_offline_no_auth_code(self):
        with self.assertRaises(BuilderException):
            self.card.authorize(14).with_currency("USD").with_offline_auth_code(
                None
            ).execute()
