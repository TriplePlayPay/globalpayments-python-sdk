...
import datetime
from python_sdk.globalpayments.api.entities import (
    Customer,
...
from python_sdk.globalpayments.api.entities.debit_mac import DebitMac
from python_sdk.globalpayments.api.entities.three_d_secure import ThreeDSecure
from python_sdk.globalpayments.api.entities.transaction_summary import (
    TransactionSummary,
)
from python_sdk.globalpayments.api.entities.enums import (
    AccountType,
    AddressType,
    AliasAction,
    CheckType,
...
from python_sdk.globalpayments.api.entities.exceptions import (
    ApiException,
    BuilderException,
    GatewayException,
    UnsupportedTransactionException,
)
from python_sdk.globalpayments.api.gateways.gateway_response import GatewayResponse
from python_sdk.globalpayments.api.payment_methods.credit import Credit, CreditCardData
from python_sdk.globalpayments.api.payment_methods.echeck import ECheck
from python_sdk.globalpayments.api.payment_methods.giftcard import GiftCard
from python_sdk.globalpayments.api.payment_methods import (
    Encryptable,
    TrackData,
...
class Gateway(object):
    ...
    def __init__(self, content_type, logger=None):
    ...
    def send_request(self, verb, endpoint, data=None, query_string_params=None):
        ...
        self.logger.info(f"Preparing to call URL: {url}")
        request_headers = {"Content-Type": self._content_type}
        for key in self.headers:
            request_headers[key] = self.headers[key]

        self.logger.info(f"Using request headers: {request_headers}")
        ...
        try:
            ...
            def __format_status_code(status_code):
                """
                This handy function formats the status
                code in such a way it avoids detection by
                the sensitive data filter.
                """
                return "_".join([s for s in str(status_code)])

            response.status_code = request.status
            response.raw_response = raw_response
            self.logger.info(
                f"status: {__format_status_code(response.status_code)}, response: {response.raw_response}"
            )
    ...
    def _build_payment_method(self, request, payment, transaction_type):
        ...
        if transaction_type == TransactionType.Create:
            ...
            if isinstance(payment.payment_method, CreditCardData):
                ...
                payment_info = {
                    "token" if has_token else "number": token_value
                    if has_token
                    else method.number,
...
class PorticoConnector(XmlGateway):
    ...
    def process_authorization(self, builder):
        ...
        if is_check or builder.billing_address is not None:
            ...
            if builder.billing_address is not None:
                ...
                et.SubElement(
                    holder, "City" if is_check else "CardHolderCity"
                ).text = builder.billing_address.city
                et.SubElement(
                ...
                et.SubElement(
                    holder, "Zip" if is_check else "CardHolderZip"
                ).text = builder.billing_address.postal_code
        ...
        if (
            ...
            if isinstance(card, CreditCardData):
                ...
                if secure_ecom is not None:
                    ...
                    et.SubElement(
                        secure_ecommerce, "PaymentDataSource"
                    ).text = secure_ecom.payment_data_source
                    et.SubElement(
                        secure_ecommerce, "TypeOfPaymentData"
                    ).text = secure_ecom.payment_data_type
                    et.SubElement(
                        secure_ecommerce, "PaymentData"
                    ).text = secure_ecom.cavv
                    et.SubElement(
                        secure_ecommerce, "ECommerceIndicator"
                    ).text = secure_ecom.eci
            ...
            if builder.transaction_type == TransactionType.Replace:
                ...
                if builder.replacement_card.pin:
                    et.SubElement(
                        new_card_data, "PIN"
                    ).text = builder.replacement_card.pin
            ...
            if not has_token:
                ...
                if check.routing_number:
                    et.SubElement(
                        account_info, "RoutingNumber"
                    ).text = check.routing_number
                if check.account_number:
                    et.SubElement(
                        account_info, "AccountNumber"
                    ).text = check.account_number
            ...
            if check.entry_mode:
                et.SubElement(
                    block1, "DataEntryMode"
                ).text = check.entry_mode.value.upper()
            if check.check_type:
        ...
        if isinstance(builder.payment_method, TransactionReference):
            ...
            if reference.client_transaction_id:
                et.SubElement(
                    block1, "ClientTxnId"
                ).text = reference.client_transaction_id
        ...
        if isinstance(builder.payment_method, RecurringPaymentMethod):
            ...
            if method.payment_type == "ACH":
                ...
                if builder.payment_method.sec_code is not None:
                    et.SubElement(
                        block1, "SECCode"
                    ).text = builder.payment_method.sec_code.value
        ...
        if self._has_attr(builder.payment_method, "pin_block"):
            if builder.transaction_type != TransactionType.Reversal:
                et.SubElement(
                    block1, "PinBlock"
                ).text = builder.payment_method.pin_block
        ...
        if self._has_attr(builder, "balance_inquiry_type"):
            et.SubElement(
                block1, "BalanceInquiryType"
            ).text = builder.balance_inquiry_type.value
        ...
        if builder.ecommerce_info is not None:
            et.SubElement(
                block1, "Ecommerce"
            ).text = builder.ecommerce_info.channel.value
            ...
            if (
                ...
                et.SubElement(
                    direct, "DirectMktInvoiceNbr"
                ).text = builder.invoice_number
    ...
    def manage_transaction(self, builder):
        ...
        if builder.transaction_type == TransactionType.TokenUpdate:
            ...
            et.SubElement(
                set_month_attribute, "Value"
            ).text = builder.payment_method.exp_month
            ...
            et.SubElement(
                set_year_attribute, "Value"
            ).text = builder.payment_method.exp_year
    ...
    def process_report(self, builder):
        ...
        if builder.timezone_conversion:
            et.SubElement(
                transaction, "TzConversion"
            ).text = builder.timezone_conversion
        ...
        if isinstance(builder, gp.api.builders.TransactionReportBuilder):
            ...
            if builder.start_date is not None:
                et.SubElement(
                    transaction, "RptStartUtcDT"
                ).text = builder.start_date.strftime("%Y-%m-%dT%H:%M:%S.%f")

            if builder.end_date is not None:
                et.SubElement(
                    transaction, "RptEndUtcDT"
                ).text = builder.end_date.strftime("%Y-%m-%dT%H:%M:%S.%f")
    ...
    def _map_response(self, raw_response, payment_method):
        ...
        if "PaymentFacilitatorTxnId" in item:
            result.payment_facilitator_transaction_id = str(
                item["PaymentFacilitatorTxnId"]
            )
        if "PaymentFacilitatorTxnNbr" in item:
            result.payment_facilitator_transaction_number = str(
                item["PaymentFacilitatorTxnNbr"]
            )
...
class RealexConnector(XmlGateway):
    ...
    def manage_transaction(self, builder):
        ...
        if builder.transaction_type == TransactionType.VerifySignature:
            et.SubElement(
                request, "payres"
            ).text = builder.payer_authentication_response
        ...
        if (
            ...
            if isinstance(builder.entity, RecurringPaymentMethod):
                ...
                if payment.payment_method is not None:
                    ...
                    et.SubElement(
                        request, "sha1hash"
                    ).text = GenerationUtils.generate_hash(self.shared_secret, to_hash)
